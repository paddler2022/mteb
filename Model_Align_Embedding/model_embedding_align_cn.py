...
def load_bilingual_dict(txt_path: str) -> Dict[str, List[str]]:

    zh_to_en = defaultdict(list)
    ...
    with open(txt_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            parts = line.split(' ', 1)
            if len(parts) == 2:
                zh_word, en_word = parts[0], parts[1]

                if zh_word.isascii():
                    continue

                if has_unsafe_chars(zh_word):
                    skipped += 1
                    continue
                zh_to_en[zh_word].append(en_word)

    print(f"Dictionary: {len(zh_to_en)} Chinese words")
    if skipped > 0:
        print(f"  (Skip {skipped} words with special characters)")

    return dict(zh_to_en)
...
def lexicon_walk_mapping(
        zh_to_en: Dict[str, List[str]],
        tokenizer,
        embed_weight: torch.Tensor
) -> Dict[str, torch.Tensor]:
    """
    Lexicon Walk Mapping (LWM)

    For every chinese word:
    1. Get its English translated word
    2. tokenize corresponding English word
    3. Average all English token embedding
    """
    new_vocab = {}

    for zh_word, en_words in zh_to_en.items():
        all_embeds = []
        ...
        if all_embeds:
            new_vocab[zh_word] = torch.stack(all_embeds).mean(dim=0)
...
def filter_new_tokens(
    ...
    skipped_count = 0
    for zh_word, embed in new_vocab.items():

        if has_unsafe_chars(zh_word):
            skipped_count += 1
            continue

        ids = tokenizer.encode(zh_word, add_special_tokens=False)
        decoded = tokenizer.decode(ids).replace(' ', '').replace('▁', '')


        if decoded != zh_word:
            words_to_add.append(zh_word)
...
def greenplm_create(
        model_name: str,
        dict_path: str,
        output_dir: str = './greenplm_output',
        device: str = 'cuda',
        verify: bool = True
):
    """
    ...
    Args:
        model_name: HuggingFace model name，like "intfloat/e5-base-v2"
    ...
    print(f"  Original tokenizer length: {len(tokenizer)}")
    ...
    print("\n[2/6] Load Dictionary...")
    zh_to_en = load_bilingual_dict(dict_path)


    print("\n[3/6] Lexicon Walk Mapping...")
    new_vocab = lexicon_walk_mapping(zh_to_en, tokenizer, original_weight)
    print(f"  Get {len(new_vocab)} Chinese embedding")


    print("\n[4/6] Expanding tokenizer...")
    words_to_add, embeds_to_add = filter_new_tokens(tokenizer, new_vocab)
    print(f"  Need to add: {len(words_to_add)} tokens")

    if len(words_to_add) == 0:
        print("  No tokens needed to be added")
    ...
    # Verify that the orginal embedding is not changed
    ...
    if verify:
        print("\n[6/6] Verifying...")
        _verify_tokenization(tokenizer, model_name)
        _verify_model_output(model, model_name, tokenizer, device)
    ...
    if os.path.exists(tokenizer_json_path):
        ...
        print("  (已删除 tokenizer.json 以确保 Fast tokenizer 兼容)")
    ...
    print(f"  (Add SentenceTransformer configuration, Normalize={has_normalize}, max_seq_length={orig_max_seq_length})")

    # 中文测试
    print("\nChinese Tokenization TEST:")
    for word in ["你好", "机器学习", "人工智能", "深度学习"]:
...
def _verify_tokenization(tokenizer, original_model_name: str):
...
def _verify_model_output(model, original_model_name: str, tokenizer, device: str):
    ...
    with torch.no_grad():
        for text in test_texts:
            ...
            if out_orig.shape != out_new.shape:
                print(f"    ✗ '{text}' shape does not match")
                continue
...
def greenplm_load(model_dir: str, device: str = 'cuda', use_fast: bool = True):
...
def greenplm_create_st(
    ...
    print(f"  Tokenizer: {type(tokenizer).__name__} (Slow)")
    print(f"  Original Size: {len(tokenizer)}")
    ...
    print("\n[3/6] Lexicon Walk Mapping...")
    new_vocab = lexicon_walk_mapping(zh_to_en, tokenizer, original_weight)
    print(f"  Create {len(new_vocab)} embedding")

    # 4. Expanding vocab sie
    ...
    # Embedding test
    print("\nChinese Embedding test:")
    test_zh = ["你好世界", "机器学习很有趣"]
    embeddings = model.encode(test_zh, convert_to_tensor=True)
...
def greenplm_load_st(model_dir: str, device: str = 'cuda'):
...
if __name__ == "__main__":
    # # E5 model（encoder-only，mo lm_head）
    # model, tokenizer = greenplm_create(
    #     model_name="intfloat/e5-large-v2",
    #     dict_path="./zh-en_simplified.txt",
    #     output_dir='./aligned_greenplm_e5_large_v2_chinese',
    #     device='cuda',
    #     verify=True
    # )
    ...
    model, tokenizer = greenplm_create(
        ...
        dict_path="./zh-en_simplified.txt",
        output_dir='./aligned_greenplm_all_MiniLM_L12_v2_chinese',
        device='cuda',
        verify=True
    )