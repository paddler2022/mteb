...
class Core17InstructionRetrievalCodeSwitching(AbsTaskRetrieval):
    """
    Core17 Instruction Retrieval Code-Switching 变体任务
    ...
    metadata = TaskMetadata(
        name="Core17InstructionRetrievalCodeSwitching",
        description="Core17 Instruction Retrieval Code-Switching variant. Measuring retrieval instruction following ability on Core17 narratives for the FollowIR benchmark.",
...
""",
    ...
    def __init__(self, query_file: str = None, instruction_file: str = None, **kwargs):
        ...
        Args:
            query_file: 本地 queries jsonl 文件路径。如果为 None，则从环境变量 CORE17_QUERY_FILE 获取
            instruction_file: 本地 instructions jsonl 文件路径。如果为 None，则从环境变量 CORE17_INSTRUCTION_FILE 获取
        """
        super().__init__(**kwargs)
        self.query_file = query_file or os.getenv("CORE17_QUERY_FILE")
        self.instruction_file = instruction_file or os.getenv("CORE17_INSTRUCTION_FILE")
    def load_data(self, **kwargs):
        ...
        if not self.query_file:
            raise ValueError(
                ...
                "Please pass query_file parameter or set CORE17_QUERY_FILE environment variable."
            )

        if not os.path.exists(self.query_file):
            raise FileNotFoundError(f"Query file not found: {self.query_file}")

        # ========== 2. 从本地加载 queries and instructions ==========
